...
class SpaceSearcher
{
 ...
 public:
  ...
  SpaceSearcher(const Box3D& bb,
                ...
                thrust::make_transform_iterator(last, t2p)){
                  // Delegating constructor using another constructor
  }
   ...
   */
  struct p2code : public thrust::unary_function<Point, code_type>{
    MortonCoderType moco_;
    p2code(MortonCoderType moco) : moco_(moco){}
  ...
  struct comp {
    bool operator()(morton_pair mp1, morton_pair mp2){
  ...
  SpaceSearcher(const Box3D& bb,
    ...
    // Using transform_iterator, construct an iterator representing the morton code
    using CodeIter = thrust::transform_iterator<p2code, PointIter, code_type>;
    CodeIter cfirst = thrust::make_transform_iterator(pfirst, p2code(mc_));
    CodeIter clast = thrust::make_transform_iterator(plast, p2code(mc_));

    // Using the zip iterator to initialize z_data_
    using TupleIter = thrust::tuple<CodeIter, TIter>;
    using ZipIter = thrust::zip_iterator<TupleIter>;
    ZipIter zfirst(thrust::make_tuple(cfirst, tfirst));
    ZipIter zlast(thrust::make_tuple(clast, tlast));
    z_data_ = std::vector<morton_pair>(zfirst, zlast);

    // Sort the z_data_ vector using the custom comparing function
    thrust::sort(thrust::omp::par, z_data_.begin(), z_data_.end(), comp());
  }
 ...
 private:
  ...
  struct morton_pair {
    ...
    morton_pair(thrust::tuple<code_type, T> values)
    : code_(thrust::get<0>(values)), value_(thrust::get<1>(values)) {}
  };
